...
From `system-architecture.md` Section 2.3 — useConnectionStore:
```typescript
...
From `ux-spec.md` Section 2.1 — Matrix color palette CSS custom properties (copy full palette).
...
### File List
- `frontend/src/App.tsx` — Updated with socket integration and ping/pong UI
- `frontend/src/hooks/useSocket.ts` — WebSocket client hook (new)
- `frontend/src/components/ConnectionStatus.tsx` — Status indicator (new)
- `frontend/src/stores/useConnectionStore.ts` — Zustand store (new)
- `frontend/tsconfig.json` — Fixed rootDir for shared/ compatibility
...
|----|----------|--------|-----------|
| 1 | React 18 + Vite em localhost:5173 | ✅ PASS | `main.tsx` — `createRoot`, `vite.config.ts` port 5173 |
| 2 | Tailwind com paleta Matrix | ✅ PASS | `tailwind.config.js` — matrix colors, fonts, shadows (Story 1.1) |
| 3 | Socket.io-client conecta em ws://localhost:3000 | ✅ PASS | `useSocket.ts:5,14` — `io(SOCKET_URL, { transports: ['websocket'] })` |
| 4 | Status "Connected"/"Disconnected" com cor | ✅ PASS | `ConnectionStatus.tsx` — green dot/text ou red dot/text |
| 5 | Botão "Send Ping" | ✅ PASS | `App.tsx:16-21` — botão com `onClick={sendPing}` |
| 6 | Exibe "Pong received at {timestamp}" | ✅ PASS | `App.tsx:23-26` — renderiza ISO timestamp do lastPong |
| 7 | Tipos SocketMessage e ConnectionStatus em shared | ✅ PASS | `shared/types.ts:206-211` (pré-existente Story 1.1) |
| 8 | Console log de conexão/desconexão | ✅ PASS | `useSocket.ts:18,24` — `console.log('WebSocket connected/disconnected')` |
| 9 | Tema escuro com texto verde | ✅ PASS | `globals.css` — `--matrix-black: #0D0D0D`, `App.tsx` — `bg-matrix-black` |
| 10 | `npm run dev` com HMR | ✅ PASS | `package.json:7` — `"dev": "vite"`, Vite HMR nativo |
...
| `vite build` | ✅ PASS | 76 modules, 1.52s, bundle 189KB gzip 60KB |

### Code Quality Analysis

**Positivo:**
- Hook `useSocket` segue boas práticas: cleanup no unmount, `useCallback` para `sendPing`, selectors individuais do Zustand
- Store `useConnectionStore` segue exatamente a interface da arquitetura Section 2.3
- `ConnectionStatus` é componente puro com selector otimizado
- Sem dependências circulares

### Concerns (não bloqueantes)

1. **SOCKET_URL hardcoded** (`useSocket.ts:5`) — `http://localhost:3000` está fixo no código. Em produção precisaria vir de env var. Severity: LOW — aceitável para esta story de validação.
2. **Pong timestamp usa `Date.now()`** em vez do timestamp ISO vindo do server (`useSocket.ts:29`). O AC 6 diz "Pong received at {timestamp}" — funciona, mas perde o timestamp do servidor. Severity: LOW — informativo.
3. **Sem testes automatizados** — A story especifica "manual validation" na seção Testing, então é aceitável. Recomendo testes em story futura.
4. **`incrementReconnect` nunca é chamado** — O store expõe `incrementReconnect()` mas `useSocket` não o utiliza nos eventos de reconnect do Socket.io. Severity: LOW — funcionalidade disponível para uso futuro.

### Verdict

**PASS** — Todos os 10 ACs implementados e verificados. TypeScript limpo, build OK. Código bem estruturado seguindo a arquitetura. Concerns são melhorias de baixa severidade que não bloqueiam a entrega.
