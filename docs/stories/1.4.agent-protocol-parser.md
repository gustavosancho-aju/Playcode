...
- [x] Task 2: Handle edge cases (AC: 4, 5)
  - [x] Missing [TASKS] block → tasks = []
  - [x] Missing [OUTPUT] block → output = null
  - [x] Missing [HANDOFF] block → handoff = null
  - [x] No [AGENT:] tag → agent = 'unknown', message = rawText
  - [x] Empty string → agent = 'unknown', message = ''
...
From `system-architecture.md` Section 4.2 — complete parser implementation:
...
**Task status mapping:** `x` → completed, `~` → in_progress, ` ` → pending
...
### File List
- `backend/src/parser/agent-parser.ts` — Parser module (new)
- `backend/tests/parser.test.ts` — 10 test cases (new)
