...
- [x] Task 1: Backend mock endpoint (AC: 1, 2)
  ...
  - [x] POST `/api/mock/agent-update` — validates body { agent, status, message }
...
From `system-architecture.md` Section 3.4 — Mock API:
```yaml
POST /api/mock/agent-update → { broadcasted: true }
     Body: { agent, status, message }
```

From Section 3.5 — Socket.io events:
```yaml
pipeline-update:
  agent: AgentId
...
From Section 2.1 — Communication between layers:
```typescript
...
**Zustand selector optimization** — each AgentHouse should use:
```typescript
...
# Sends: master→processing, master→done, pesquisa→processing, pesquisa→done, ...
```
...
- Test script sends sequential processing→done for all 8 agents

### File List
- `backend/src/routes/mock.ts` — Mock agent-update endpoint (new)
- `backend/src/index.ts` — Registers mock router with io instance
- `frontend/src/hooks/useSocket.ts` — Added pipeline-update listener + reconnection config
- `frontend/src/components/ConnectionStatus.tsx` — Added reconnection counter
- `scripts/test-websocket.js` — 8-agent sequential test script (new)
...
|----|----------|--------|-----------|
| 1 | Mock endpoint POST /api/mock/agent-update | ✅ PASS | `mock.ts:8` — validates agent+status, returns `{ broadcasted: true }` |
| 2 | Broadcast via WebSocket | ✅ PASS | `mock.ts:18` — `io.emit('pipeline-update', payload)` |
| 3 | Frontend listener updates Zustand | ✅ PASS | `useSocket.ts:51-58` — `pipeline-update` → `useAgentStore.getState().updateAgent()` |
| 4 | AgentHouse reacts (border, glow, message) | ✅ PASS | AgentHouse component uses status-driven styles (from Story 1.5) |
| 5 | Latency < 500ms | ✅ PASS | Socket.io local WebSocket + Zustand direct update = near-instant |
| 6 | Rapid updates no jank | ✅ PASS | `useAgentStore.getState()` avoids React render queue for external updates |
| 7 | Reconnection within 3s | ✅ PASS | `useSocket.ts:27-28` — `reconnectionDelay: 1000`, max 10 attempts |
| 8 | Test script sends 8 updates | ✅ PASS | `scripts/test-websocket.js` — 8 sequential agents |
| 9 | Houses light up sequentially | ✅ PASS | Test script with 2s delay per update |
| 10 | Reconnection counter displayed | ✅ PASS | `ConnectionStatus.tsx:17-20` — shows `(reconnecting... N)` |
...
| `jest` (backend) | ✅ PASS (14/14) |
| `vite build` (frontend) | ✅ PASS |

### Concerns (não bloqueantes)

1. **SOCKET_URL hardcoded** (`useSocket.ts:7`) — `http://localhost:3000` é hardcoded. Deveria usar variável de ambiente (`VITE_WS_URL`). Severity: MEDIUM — funciona para dev, mas impede deploy flexível.
2. **Mock endpoint sem validação de AgentId** (`mock.ts:11`) — Aceita qualquer string como `agent`, sem validar contra `AGENT_DEFINITIONS`. Uma string inválida seria ignorada silenciosamente no frontend. Severity: LOW.
3. **`data.status as AgentStatus` cast inseguro** (`useSocket.ts:54`) — Não valida se o status recebido é um valor válido de AgentStatus. Severity: LOW — mock endpoint é controlado.

### Verdict

**PASS** — Todos os 10 ACs verificados. Integração WebSocket end-to-end funcional. Concerns de baixa severidade.
