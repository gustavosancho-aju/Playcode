...
From `ux-spec.md` Section 11.1 — Code Rain specs:
```
...
From `ux-spec.md` Section 2 — Complete Design System:
...
From `ux-spec.md` Section 3.1 — Dashboard wireframe with bottom bar layout.
...
### File List
- `frontend/src/components/MatrixRain.tsx` — Canvas code rain (new)
- `frontend/src/components/Title.tsx` — Glowing title (new)
- `frontend/src/components/LoadingScreen.tsx` — Connection loading (new)
- `frontend/src/components/BottomBar.tsx` — Bottom toolbar (new)
- `frontend/src/stores/useEffectsStore.ts` — Effects/motion store (new)
- `frontend/src/App.tsx` — Integrated all new components
- `frontend/public/favicon.svg` — Matrix favicon (new)
...
|----|----------|--------|-----------|
| 1 | Matrix code rain Canvas, 30fps+ | ✅ PASS | `MatrixRain.tsx` — Canvas com requestAnimationFrame, katakana+latin+digits |
| 2 | Global CSS: bg-matrix-black, text-white, accents | ✅ PASS | `index.html:10` body classes, globals.css pre-existing from 1.1/1.3 |
| 3 | Typography: Inter, JetBrains Mono, Press Start 2P | ✅ PASS | `index.html:7` — Google Fonts CDN link |
| 4 | Glow effects on active elements | ✅ PASS | shadow-glow classes, AgentHouse border glow (from 1.5) |
| 5 | Status indicators styled | ✅ PASS | StatusBadge atom from 1.5 with gray/yellow/green |
| 6 | Connector paths green neon | ✅ PASS | Dashboard connectors from 1.5 |
| 7 | Page title "SQUAD PLAYGROUND" Press Start 2P | ✅ PASS | `Title.tsx` — font-arcade class com green glow text-shadow |
| 8 | Favicon 32x32 green matrix icon | ✅ PASS | `favicon.svg` — green "S" on black, `index.html:5` referência |
| 9 | Loading state "CONNECTING..." animated | ✅ PASS | `LoadingScreen.tsx` — pulse animation while disconnected |
| 10 | 60fps scroll with Matrix rain | ✅ PASS | requestAnimationFrame + cancelAnimationFrame cleanup |
...
| `vite build` (frontend) | ✅ PASS (86 modules, 197KB) |

### Concerns (não bloqueantes)

1. **MatrixRain não throttle a 30fps** (`MatrixRain.tsx`) — Usa requestAnimationFrame nativo (~60fps). UX spec pede 30-40fps para eficiência. Sem throttle, consome mais CPU. Severity: MEDIUM — funcional, mas pode impactar bateria em notebooks.
2. **useEffectsStore intensity states não implementados** — UX spec define idle/processing/victory com diferentes contagens de colunas e opacidade. Implementação atual usa contagem fixa. Severity: LOW — funcional para MVP.
3. **BottomBar botões são placeholders** — Pause, Download, Settings, Help não têm funcionalidade. Severity: LOW — esperado conforme story (placeholder buttons).
4. **LoadingScreen usa `useConnectionStore`** — Se o WebSocket nunca conectar, a tela de loading fica permanente sem timeout. Severity: LOW — comportamento esperado para dev.

### Verdict

**PASS** — Todos os 10 ACs verificados. Tema Matrix implementado com Canvas rain, fontes, glow effects, favicon e loading screen. Concerns não bloqueantes.
